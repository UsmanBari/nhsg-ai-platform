...
* **Implementation Decision**: The implementation explicitly treats any income inside $[47,000, 53,000]$ PKR as non-binding (`income_side = "unknown"`). Rule R5 (`REJECT_INELIGIBLE_INCOME`) requires a binding `income_side == "above_threshold"`. In `CASE-003` (Gross 52,000, Net 48,500), because the applicant also submitted an explicit exception request, Rule R4 (`ESCALATE_REQUIRES_HUMAN`) fires.
* **Engineering Rationale**: Financial figures sitting within the numeric margin band cannot bind an automated income rejection. Setting `income_side = "unknown"` ensures Rule R5 does not auto-reject applicants whose salary figures fall inside the uncertainty margin.
...
| **2** | Income threshold PKR 50k $\pm$ PKR 3k margin. | Does not specify decision code for income inside $[47\text{k}, 53\text{k}]$ without exception. | Income in $[47\text{k}, 53\text{k}]$ classified as `unknown`; prevents R5 auto-rejection. | Boundary figures are non-binding; protects grant pool from improper machine decisions. |
